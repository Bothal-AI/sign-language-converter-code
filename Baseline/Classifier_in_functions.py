    ...
    while True:
        ...
        if hands:
            ...
            if heightOverWidth > 1:  # if image height is larger than width
                ...
                resizedImage = cv2.resize(imageCropped, (calculatedWidth, imageSize))  # resize the image
                ...
                prediction, index = classifier.getPrediction(imageWithBackground)  # get prediction and index value

            else:   # if image width is larger than height
                constant = imageSize / w
                calculatedHeight = math.ceil(constant * h)
                resizedImage = cv2.resize(imageCropped, (imageSize, calculatedHeight))  # resize the image
            ...
            # styling output
            cv2.rectangle(imageOutput, (x - offsetValue, y - offsetValue - 50), (x - offsetValue + 90, y - offsetValue),
                          ((0, 255, 0)), cv2.FILLED)
            cv2.putText(imageOutput, labels[index], (x, y - 30), cv2.FONT_HERSHEY_DUPLEX, 1.5, (0, 0, 0), 2)
            cv2.rectangle(imageOutput, (x - offsetValue, y - offsetValue), (x + w + offsetValue, y + h + offsetValue),
                          ((0, 255, 0)), 3)
